...
def AU(k, wf='av18', nff='ba'):
    ...
    Pass wf='av18', 'paris' or 'cdbonn' to select wavefunction.

    Pass nff='ba', 'mab', 'hz', 'mit' or 'point' to select nucleon form factors.
    '''
    u, w, *_ = choose_wf(wf)
    AN, *_ = choose_nff(nff)
    return _AU(k, u=u, w=w, AN=AN(k))
...
def DU(k, wf='av18', nff='ba'):
    ...
    return _DU(k, u=u, w=w, u1=u1, w1=w1, u2=u2, w2=w2, AN=AN(k), JN=JN(k), DN=DN(k))
...
def DT1(k, wf='av18', nff='ba'):
    ...
    u, w, u1, w1, u2, w2, _, _ = choose_wf(wf)
    AN, JN, DN, *_ = choose_nff(nff)
    return _DT1(k, u=u, w=w, u1=u1, w1=w1, u2=u2, w2=w2, AN=AN(k), JN=JN(k), DN=DN(k))
...
def DT2(k, wf='av18', nff='ba'):
    ...
    return _DT2(k, u=u, w=w, u1=u1, w1=w1, u2=u2, w2=w2, AN=AN(k), JN=JN(k))
...
def cU(k, wf='av18', nff='ba'):
    ...
    # Need to change rmin from 0 to 1e-2 for the Paris & CD Bonn wave functions,
    # because of an instability at small r
    rmin = 0
    if(wf=='paris' or wf=='cdbonn'):
        rmin = 1e-2
    return _cU(k, u=u, w=w, u1=u1, w1=w1, u2=u2, w2=w2, u3=u3, w3=w3, AN=AN(k), cN=cN(k), rmin=rmin)
...
def cT1(k, wf='av18', nff='ba', formula='cT1'):
    ...
    return _cT1(k, u=u, w=w, u1=u1, w1=w1, u2=u2, w2=w2, u3=u3, w3=w3, AN=AN(k), JN=JN(k), cN=cN(k), formula=formula)
...
def cT2(k, wf='av18', nff='ba', formula='cT2'):
    ...
    if(wf=='paris' or wf=='cdbonn'):
        rmin =  1e-2
    return _cT2(k, u=u, w=w, u1=u1, w1=w1, u2=u2, w2=w2, u3=u3, w3=w3, AN=AN(k), JN=JN(k), rmin=rmin, formula=formula)
...
def J(k, wf='av18', nff='ba', formula='form1'):
    ...
    AN, JN, *_ = choose_nff(nff)
    return _J(k, u=u, w=w, u1=u1, w1=w1, AN=AN(k), JN=JN(k), formula=formula)
...
def sbar(k, wf='av18', nff='ba', formula='sbar'):
    ...
    u, w, u1, w1, u2, w2, u3, w3 = choose_wf(wf)
    _, _, _, _, SN = choose_nff(nff)
    return _sbar(k, u=u, w=w, u1=u1, w1=w1, u2=u2, w2=w2,u3=u3,w3=w3, SN=SN(k), formula=formula)
...
def _AU_integrand(r, k, u, w, AN):
    kfm = k/hbar
    intd = AN * (u(r)**2 + w(r)**2)/2 * jn(0, kfm*r/2)
    return intd

def _AT_integrand(r, k, u, w, AN):
    kfm = k/hbar
    intd = AN * jn(2, kfm*r/2)*(
...
def _cU_integrand(r, k, u, w, u1, w1, u2, w2, u3, w3, AN, cN):
    kfm = k/hbar
    A1_piece = AN/(2*kfm*(mN/hbar)**2) * jn(1,kfm*r/2)*(
            2*u1(r)*u2(r) + 2*w1(r)*w2(r)
            - 12*w(r)**2/r**3
            )
    A02_piece = AN/(12*(mN/hbar)**2)*(jn(0,kfm*r/2)-2*jn(2,kfm*r/2))*(
            u(r)*u2(r) + w(r)*w2(r)
            )
    c_piece = 0.5*cN*jn(0,kfm*r/2)*(u(r)**2 + w(r)**2)
    intd = A1_piece + A02_piece + c_piece
    return intd

def _cT1_integrand(r, k, u, w, u1, w1, u2, w2, u3, w3, AN, JN, cN):
    kfm = k/hbar
    A3_piece = 6*AN/kfm**3 * jn(3,kfm*r/2)*(
    ...
    A24_piece = 6*AN/(14*kfm**2)*(3*jn(2,kfm*r/2)-4*jn(4,kfm*r/2))*(
                np.sqrt(2)*(u(r)*w2(r) + w(r)*u2(r))
                - w(r)*w2(r)
                )
    c_piece = 6*mN**2/k**2 * cN * jn(2,kfm*r/2)*(
            2*np.sqrt(2)*u(r)*w(r) - w(r)**2
            )
    # TODO: determine whether this J piece is right or too big by a factor 2
    J_piece = 6*np.sqrt(2)*JN*jn(2,kfm*r/2)/kfm**2*(
...
def _cT1_integrandAlan(r, k, u, w, u1, w1, u2, w2, u3, w3, AN, JN, cN):
    ...
    J3_piece = 3*JN/(5*np.sqrt(2)*r*kfm)*jn(3,kfm*r/2)*(
           3*(r*u1(r)*w(r)-r*w1(r)*u(r)+2*w(r)*u(r))
           )
    J1_piece = 3*JN/(5*np.sqrt(2)*r*kfm)*jn(1,kfm*r/2)*(
...
def _cT2_integrand(r, k, u, w, u1, w1, u2, w2, u3, w3, AN, JN):
    ...
    intd = 3*AN/(mN**2*k**2)*hbar**4 * (A2_piece + A13_piece)
    J_piece = 3*np.sqrt(2)*JN*jn(2,kfm*r/2)/(2*mN/hbar)**2*(
...
def _cT2_integrandAlan(r, k, u, w, u1, w1, u2, w2, u3, w3, AN, JN):
    ...
    intd = -AN/(140*mN**2*r**2)*hbar**2 * (A0_term + A2_term+A4_term)
    # J terms (confirmed)
    J3_piece = -3*JN*k*hbar/(20*np.sqrt(2)*mN**2*r)*jn(3,kfm*r/2)*(
           3*(r*u1(r)*w(r) - r*u(r)*w1(r)+2*u(r)*w(r))
           )
    J1_piece = -3*JN*k*hbar/(20*np.sqrt(2)*mN**2*r)*jn(1,kfm*r/2)*(
...
def _cT2_integrandAlan3(r, k, u, w, u1, w1, u2, w2, u3, w3, AN, JN):
    ...
    intd = -3*AN/(10.0*mN**2*k*r**3)*hbar**3 * (A2_term + A13_term)
    return intd
...
def _cT2_integrandAdam3(r, k, u, w, u1, w1, u2, w2, u3, w3, AN, JN):
    ...
    intd =  3*AN/(mN**2*k**2)*hbar**4 * (A2_term)
    return intd

def _J_integrand(r, k, u, w, u1, w1, AN, JN):
    kfm = k/hbar
    A_piece = 9/2*AN/kfm * jn(1,kfm*r/2) * w(r)**2/r
    J0_piece = JN*jn(0,kfm*r/2)*(u(r)**2 - w(r)**2/2)
    J2_piece = JN*jn(2,kfm*r/2)*(w(r)**2 + np.sqrt(2)*u(r)*w(r))/2
...
def _S_integrand(r, k, u, w, SN):
    kfm = k/hbar
    S0_piece = SN*jn(0,kfm*r/2)*(u(r)**2 - w(r)**2/2)
    S2_piece = SN*jn(2,kfm*r/2)*(w(r)**2 + np.sqrt(2)*u(r)*w(r))/2
    intd = S0_piece + S2_piece
    return intd

def _sbar_integrandAdam(r, k, u, w,u1,w1,u2,w2, SN):
    kfm = k/hbar
    intd = 6*np.sqrt(2)*SN/kfm**2*jn(2,kfm*r/2)*(
...
def _sbar_integrandAlan(r, k, u, w,u1,w1 ,u2,w2,SN):
    kfm = k/hbar
    J3_piece = 3*SN/(5*np.sqrt(2)*kfm*r)*jn(3,kfm*r/2)*(
           3*(r*u1(r)*w(r) - r*u(r)*w1(r)+2*u(r)*w(r))
           )
    J1_piece = 3*SN/(5*np.sqrt(2)*kfm*r)*jn(1,kfm*r/2)*(
           2*(-r*u1(r)*w(r) + r*u(r)*w1(r)+3*u(r)*w(r))
           )
    intd = J1_piece + J3_piece
    return intd
